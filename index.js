...
async
function sendSMS(offset, wtype, windows) {
    const msg = {
...
async
function check(page, browser) {

    let foundAvailability = false;
    let foundNothing = true;
    ;
    const texts = await
    page.$$eval('.ufss-date-select-toggle-text-availability',
    ...
    if (foundAvailability) {
        ...
        try {
            texts = await
            page.$$eval('.ufss-slot.ufss-available .ufss-slot-time-window-text',
                nodes = > nodes.map(n = > n.innerText
        ))
            ;
            texts = texts.join('\n');
        ...
        await
        Promise.delay(1000 * 60 * 60 * DELAY_AFTER_FINDING_MINS); // Wait before doing this again.
    } else {
        console.log(`Looking again in ${REFRESH_INTERVAL_MINS} minutes`);
        await
        Promise.delay(1000 * 60 * REFRESH_INTERVAL_MINS); // check again in X minutes.
    }
...
async
function dealWithShit(page, browser) {
    await
    Promise.delay(1000 * 3);
    try {
        const title = await
        page.title();
        ...
        if (title === 'Substitution preferences' || title === "Before you checkout") {
            try {
                await
                page.waitForSelector('.a-button-input');
                await
            ...
            try {
                await
                page.waitForSelector('.a-button-input', {timeout: 6000});
                const buttons = await
                page.$$('.a-button-input');
                ...
                await
                Promise.all(buttons.map((button, i) = > {
                ...
                await
                page.waitForSelector('a[name="proceedToCheckout"]');
                await
                page.click('a[name="proceedToCheckout"]');
            } catch (e) {
            ...
            fs.writeFileSync('./cookies.json', JSON.stringify(cookies, null, 2));
            await
            page.goto('https://www.amazon.com/gp/cart/view.html?ref_=nav_cart', {
                waitUntil: "domcontentloaded"
            });
            ...
            await
            page.waitForSelector('.ufss-overview-container', {timeout: 1000 * 60 * 5});
            await
            check(page, browser);
        } else if (title === "Amazon Sign-In") {
            try {
                await
                page.waitForSelector('#continue', {timeout: 1000});
                if (AMAZON_EMAIL) {
                    await
                    page.type('#ap_email', AMAZON_EMAIL);
                    await
                    ...
                    page.focus('#ap_email');
                    await
                    Promise.delay(1000 * 60 * 1);
                }

            } catch (e) {
                try {
                    await
                    page.waitForSelector('#image-captcha-section', {timeout: 1000});
                    console.log('SOLVE THIS CAPCHA! in 1 minute and click signin');
                    await
                    Promise.delay(1000 * 60 * 1);
                } catch (e) {
                    await
                    page.waitForSelector('#ap_password');
                    await
                    page.click('input[name="rememberMe"]');
                    if (AMAZON_PASSWORD) {
                        await
                        page.type('#ap_password', AMAZON_PASSWORD);
                        await
                        page.click('#signInSubmit');
                    } else {
                        console.log("!!! Login to the site and wait for script, you have 1 minute.");
                        await
                        page.focus('#ap_password');
                        await
                        Promise.delay(1000 * 60 * 1);
                    }
                }
            ...
            await
            Promise.delay(1000 * 3); // nothing happens on this page...
        } else {
